...
describe("Recipe model", () => {
    ...
    beforeEach(() =>
        Recipe.sync({ force: true }).catch((err) => console.log(err))
    )

    describe("edge cases", () => {
        it("should not save an empty recipe", (done) => {
            Recipe.create({})
                .then(() =>
        ...
        it("should save a recipe if the minimum required information is provided", (done) => {
            Recipe.create(minRecipe)
                .then(() => done())
                .catch(() =>
                    done(
                        new Error(
                            "It should save a recipe if only name and summary are provided"
                        )
                    )
                )
        })

        it("should save a recipe even if extra fields are provided", (done) => {
            let extraInfo = { ...recipe1, extra: "info", being: "provided" }
            Recipe.create(extraInfo)
                .then(() => done())
                .catch(() =>
                    done(
                        new Error(
                            "Extra fields should not prevent a recipe from being created"
                        )
                    )
                )
        })
    ...
    describe("name", () => {
        ...
        it("should work when its a valid name", (done) => {
            Recipe.create(recipe1)
                ...
                .then(() => done())
                .catch(() => done(new Error("Valid recipes should be created")))
        })
    })
        ...
        it("should work with valid instructions", (done) => {
            Recipe.create(recipe1)
                ...
                .catch(() =>
                        ...
                        new Error(
                            "Recipes with valid instructions should be created"
                        )
                    )
                )
        })
